...
private:
    void topic_callback(const collect_msgs::msg::CollectMsg::SharedPtr msg) {
        if (msg->collect_msg[0].total_time > 0.0f) {
            ...
            int object_count = msg->collect_msg[0].count;
            ...
            if (outfile_.is_open()) {
                outfile_ << count_ << "\t" << total_time_str.str()
                        << "\t" << detection_time_str.str() << "\t" << positioning_time_str.str() << "\t" << object_count << std::endl;
                RCLCPP_INFO(this->get_logger(), "Count: %d\t, Move time: %s\t, Detect time: %s\t, Positioning time: %s\t, Object count: %d", count_,
                            total_time_str.str().c_str(), detection_time_str.str().c_str(), positioning_time_str.str().c_str(), object_count);
            }
        }
    }
