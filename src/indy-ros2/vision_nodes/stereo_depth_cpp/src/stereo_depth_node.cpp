...
public:
    StereoDepthNode() : Node("stereo_depth_node") {
        ...
        full_seg_pub  = create_publisher<sensor_msgs::msg::Image>("/full_ref_img", 10);
...
private:
    ...
    cv::Mat left_clean_roy,  left_clean_green, full_left_clean;
    ...
    rclcpp::Publisher<sensor_msgs::msg::Image>::SharedPtr           full_seg_pub;
...
*/
    ...
    void process_stereo() {
        while (!stop_thread_.load() && rclcpp::ok()) {
            ...
            cv::Mat total_mask = cv::Mat::zeros(left_bgr.size(), CV_8U);
            for (const auto& p : disparity_mask_pairs) {
                cv::bitwise_or(total_mask, p.left, total_mask);
            }
            ...
            cv::bitwise_and(left_full_, total_mask, full_left_clean);
            full_seg_pub->publish(*cv_bridge::CvImage(
                pair->first->header, sensor_msgs::image_encodings::MONO8,
                full_left_clean).toImageMsg());

            cv::imwrite("full_left_clean.png", full_left_clean);
