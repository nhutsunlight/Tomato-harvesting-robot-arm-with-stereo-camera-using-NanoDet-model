...
public:
    ...
    Tomato3DDetector() : Node("tomato_3d_detector") {
        ...
        full_left_img_sub_ = create_subscription<sensor_msgs::msg::Image>(
            "/full_ref_img", 10,
            std::bind(&Tomato3DDetector::full_left_camera_callback, this, _1));
...
private:
    ...
    std::vector<Eigen::Vector3f> stem_points_;
    ...
    cv::Mat left_img_, obs_img_, full_left_img_;
    ...
    rclcpp::Subscription<sensor_msgs::msg::Image>::SharedPtr            full_left_img_sub_;
    ...
    void full_left_camera_callback(const sensor_msgs::msg::Image::ConstSharedPtr& msg) {
        try {
            auto img = cv_bridge::toCvCopy(msg, "mono8")->image;
            std::lock_guard<std::mutex> lock(image_mtx_);
            full_left_img_ = img;
...
//        }
        ...
        RCLCPP_WARN(get_logger(), "Check11111, skip_disparity_: %d, first_run_: %d", skip_disparity_, first_run_);
    ...
    void process_next_tomato() {
        ...
        for (const auto& t : tomato_list_) {
            ...
            yp.x1    = t.x1;      yp.y1    = t.y1;      yp.x2   = t.x2;       yp.y2 = t.y2;
    ...
    cv::Point findNearestEmptyRegionCenter(
        const cv::Mat& mask,
        int x,
        int y,
        float min_clearance = 100.0f,
        int   min_area      = 500)
    {
        cv::Point target(x, y);
        ...
        cv::Mat obstacle, free_space;
        cv::threshold(mask, obstacle, 0, 255, cv::THRESH_BINARY);
        cv::bitwise_not(obstacle, free_space);
        ...
        cv::Mat dist;
        cv::distanceTransform(free_space, dist, cv::DIST_L2, 5);

        // ==========================
        // Safe region: dist > min_clearance
        // ==========================

        cv::Mat safe_mask;
        cv::threshold(dist, safe_mask, min_clearance, 255, cv::THRESH_BINARY);
        safe_mask.convertTo(safe_mask, CV_8U);
        ...
        int num_labels = cv::connectedComponentsWithStats(
            safe_mask, labels, stats, centroids, 8, CV_32S);
        ...
        for (int i = 1; i < num_labels; ++i)
        {
            ...
            double distance =
                cv::norm(center - target);

            double clearance =
                dist.at<float>(center.y, center.x);

            double score =
                area *
                std::pow(clearance, 1.5) /
                (distance + 1.0);
        ...
        RCLCPP_INFO(
            get_logger(),
            "Best empty center: (%d,%d), score=%.2f",
            best_center.x, best_center.y, best_score);
    ...
    std::pair<float, float> computeRollPitchFromGap(
        int center_x,
        int center_y,
        int empty_x,
        int empty_y,
        ...
        float cy)
    {
        ...
        // Tâm bbox (object)
        float X_obj = (center_y - cy_) * z_obj / fy_;
        float Y_obj = -(center_x - cx_) * z_obj / fx_;
        float Z_obj = z_obj;

        // Tâm khoảng trống
        float X_set = (empty_y - cy) * 2 * z_obj / fy;
        float Y_set = -(empty_x - cx) * 2 * z_obj / fx;
        float Z_set = z_obj/2;

        RCLCPP_INFO(get_logger(),
            "Object 3D: (%.2f, %.2f, %.2f), Empty 3D: (%.2f, %.2f, %.2f)",
            X_obj, Y_obj, Z_obj, X_set, Y_set, Z_set);

        // ===== Vector từ object -> khoảng trống =====
        Eigen::Vector3f normal(
            X_obj - X_set,
            Y_obj - Y_set,
            Z_obj - Z_set);
        ...
        // ===== Roll / Pitch =====
        float roll, pitch;
        roll  = -std::atan2(normal(1), normal(2)) * z_obj;
        pitch =  std::atan2(normal(0), normal(2)) * z_obj;
    ...
    tomato_octomap_msgs::msg::TomatoOctomaps buildTomatoOctomapMessage(
        const std::vector<Eigen::Vector3f>& primary_points,
        const std::vector<Eigen::Vector3f>& fallback_points)
    {
        tomato_octomap_msgs::msg::TomatoOctomaps msg_out;

        const std::vector<Eigen::Vector3f>* points = nullptr;
        if (!primary_points.empty()) {
            points = &primary_points;
        } else if (!fallback_points.empty()) {
        ...
        to.idx = 0;
        to.octomap = buildOctomap(*points);
        ...
        const std::vector<Eigen::Vector3f>& input_points)
    {
        ...
        // =====================================================
        // Convert → PCL cloud (in-place, no reserve loop)
        // =====================================================
        ...
        {
            cloud->points[i].x = input_points[i].x();
            cloud->points[i].y = input_points[i].y();
            cloud->points[i].z = input_points[i].z();
        }

        // =====================================================
        // Voxel downsample — giảm số lượng point
        // =====================================================
        pcl::PointCloud<PointT>::Ptr voxel_cloud(new pcl::PointCloud<PointT>);
        {
            pcl::VoxelGrid<PointT> vg;
            vg.setInputCloud(cloud);
            vg.setLeafSize(0.001f, 0.001f, 0.001f);  // tăng leaf size → ít point hơn, nhanh hơn
        ...
        pcl::PointCloud<PointT>::Ptr filtered(new pcl::PointCloud<PointT>);
        {
            ...
            sor.setMeanK(20);           // giảm từ 10 → 5, nhanh hơn ~2x
    ...
    {
        RCLCPP_WARN(get_logger(), "Running computeFromTracked...");
        if (tracked_store_.empty()) return std::nullopt;

        RCLCPP_WARN(get_logger(), "Check22222, tracked_store_ size: %zu", tracked_store_.size());
        ...
        cv::Mat full_left_img_snapshot;
        {
            std::lock_guard<std::mutex> lock(image_mtx_);
            full_left_img_snapshot = full_left_img_;
        }

        cv::Mat full_mask;
        cv::threshold(full_left_img_snapshot, full_mask, 1, 255, cv::THRESH_BINARY);
        ...
        RCLCPP_WARN(get_logger(), "Check0000000000, new_dets size: %zu", new_dets.size());
        ...
        for (int i = 0; i < n_new; ++i) {
            ...
            for (int pi = 0; pi < h; pi += 5) {
                ...
                //const uchar*     row_full_mask = full_mask.ptr<uchar>(pi);
                ...
                for (int pj = 0; pj < w; pj += 5) {
                    ...
                    if ((pi % 10 == 0) && (pj % 10 == 0) && !octomap_builded_) {  
...
//                            valid_obs_point_only_.emplace_back(pt_obs[2], -pt_obs[0], -pt_obs[1]);
            ...
            cv::Point empty_center = findNearestEmptyRegionCenter(full_left_img_snapshot, cx_det, cy_det);

            auto [roll_free, pitch_free] = computeRollPitchFromGap(cx_det, cy_det, empty_center.x, empty_center.y,
                                Z, fx_, fy_, cx_, cy_);

            float roll = matched.roll;
            float pitch = matched.pitch;
            float yaw = matched.yaw;

            if (std::abs(roll - roll_free) > M_PI/18.f)   roll = roll_free;
            if (std::abs(pitch - pitch_free) > M_PI/18.f) pitch = pitch_free;
            //if (std::abs(roll) > M_PI/4.f)  roll = M_PI/4.f * roll / std::abs(roll);
            //if (std::abs(pitch) > M_PI/4.f) pitch = M_PI/4.f * pitch / std::abs(pitch);
        ...
        stem_points_ = extractStemPoints(valid_points_obs_all_);
        
        auto msg_out = buildTomatoOctomapMessage(stem_points_, valid_points_obs_all_);
        ...
        const yolov8_msgs::msg::Yolov8Inference& msg)
    {
        ...
        for (const auto& detection : msg.yolov8_inference) {
            ...
            if (detection.id != 2 || limit > 10) continue;
            ...
            RCLCPP_INFO(get_logger(), "ROI raw: x1=%d y1=%d x2=%d y2=%d w=%d h=%d id=%ld",
                x1, y1, x2, y2, w, h, detection.id);
            if (x1 >= x2 || y1 >= y2) continue;
...
*/
            ...
            const int exp_y1 = 4*y1 - 3*y2, exp_y2 = 4*y2 - 3*y1;
            const int exp_x1 = 4*x1 - 3*x2, exp_x2 = 4*x2 - 3*x1;
            ...
            for (int i = 0; i < h; i += 5) {
                ...
                //const uchar*     row_full_mask = full_mask.ptr<uchar>(i);
                ...
                for (int j = 0; j < w; j += 5) {
                    ...
                    //if (!in_expanded && sparse && ) {
                    if (row_mask[j] <= 0 && !in_bbox && !octomap_builded_) {
                        ++count;
                        valid_points_obs_all_.emplace_back(pt_obs[2], -pt_obs[0], -pt_obs[1]);
            ...
            //float pitch_threshold_ = std::atan2(std::abs(exp_y1 - exp_y2) * Z / fy_ / 2, Z);
            //float roll_threshold_  = std::atan2(std::abs(exp_x1 - exp_x2) * Z / fx_ / 2, Z);
            //if (pitch_threshold_ >= M_PI/4.f) pitch_threshold_ = M_PI/4.f;
            //if (roll_threshold_ >= M_PI/4.f) roll_threshold_ = M_PI/4.f;
            ...
            //if (!stem_points_.empty())
            //    octomap_map_temp[detection_idx] = buildOctomap(stem_points_);

            //const float yaw_contour = computeYawFromContour2D(contour).value_or(0.f);

            cv::Point empty_center = findNearestEmptyRegionCenter(full_left_img_snapshot, center_x, center_y);

            auto [roll_free, pitch_free] = computeRollPitchFromGap(center_x, center_y, empty_center.x, empty_center.y,
                                Z, fx_, fy_, cx_, cy_);
            auto [roll_obj, pitch_obj, yaw_obj] = computeSurfaceOrientation(filtered,     0.01f, true);
            //auto [roll_obj, pitch_obj, yaw_obj] = computeSurfaceOrientation(filtered,     0.01f, false);
            auto [roll_obs, pitch_obs, yaw_obs] = computeSurfaceOrientation(filtered_obs, 0.01f, false);

            float roll  = (std::abs(roll_obs)  > M_PI/18.f) ? roll_obs  : roll_obj;
            float pitch = (std::abs(pitch_obs) > M_PI/18.f) ? pitch_obs : pitch_obj;
            if (std::abs(roll - roll_free) > M_PI/18.f)   roll = roll_free;
            if (std::abs(pitch - pitch_free) > M_PI/18.f) pitch = pitch_free;
            //if (std::abs(roll) > M_PI/4.f)  roll = M_PI/4.f * roll / std::abs(roll);
            //if (std::abs(pitch) > M_PI/4.f) pitch = M_PI/4.f * pitch / std::abs(pitch);
            ...
            RCLCPP_INFO(get_logger(),
                "Gap orient  roll=%.2f° pitch=%.2f°",
                roll_free*180/M_PI, pitch_free*180/M_PI);
            RCLCPP_INFO(get_logger(),
                "Contour yaw=%.2f°",
                yaw_obj*180/M_PI);
            RCLCPP_INFO(get_logger(),
            ...
            if (!skip_target_) {
                ...
                pi_info.x1       = x1;        pi_info.y1      = y1;        pi_info.x2     = x2;        pi_info.y2 = y2;
                poses.push_back(pi_info);
            }
            ++detection_idx;
        }
